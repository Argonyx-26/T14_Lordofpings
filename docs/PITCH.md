...
> factor. Claude writes the brief, but it can't create or hide an incident, and we reject any sentence that isn't
...
| Does the LLM hallucinate threats? | "It can't: detection and scoring are deterministic; Claude only writes the explanation, we reject anything not backed by the evidence, and there's a template fallback if it's offline." |
